...
TRAIN_MANIFEST="manifests/curriculum_gray3_stageb_v1_difficulty/train_B1A.csv"
VAL_MANIFEST="manifests/curriculum_gray3_stageb_v1_difficulty/val_B1A.csv"
SAVE_DIR="experiments/curriculum_gray3_stageB_v1_B1A_difficulty_conservative"
...
if [[ ! -f "$TRAIN_MANIFEST" || ! -f "$VAL_MANIFEST" ]]; then
  echo "[FATAL] missing StageB manifest(s); run scripts/curriculum_gray3/04_build_stageB_v1_difficulty_manifests.py first" >&2
  exit 2
fi

# cheap manifest preflight: row counts, missing paths, leakage checks are enforced by builder.
python3 scripts/curriculum_gray3/04_build_stageB_v1_difficulty_manifests.py > /tmp/stageb_v1_manifest_preflight.json
