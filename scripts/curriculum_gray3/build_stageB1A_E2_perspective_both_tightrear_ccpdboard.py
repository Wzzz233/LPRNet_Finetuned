#!/usr/bin/env python3
"""Build StageB1A-E2 manifests from the new perspective-both/tightrear dataset.

Definition:
- Start from original StageB1A difficulty manifests.
- Replace only train source=green_edgefit_extreme rows with E2 generated ccpd_board rows.
- Keep original validation/proxy files unchanged for old benchmark.
- Produce a separate new-proxy manifest dir replacing only proxy_green_edgefit_extreme with non-overlapping E2 proxy rows.
"""
import csv
import json
import os
...
DATA_ROOT = ROOT / 'tmp/green_extreme_stageB1A_E2_perspective_both_tightrear_20260427'
OUT_DIR = ROOT / 'manifests/curriculum_gray3_stageb_v1_B1A_E2_perspective_both_tightrear_eval_original'
...
EXTRA_FIELDS = [
    ...
    'ocr_crop_mode', 'ocr_resize_mode', 'ocr_resize_kernel', 'ocr_preproc', 'ocr_channel_order',
    'ocr_min_occ_ratio', 'ocr_quad_pad_ratio', 'difficulty_tier', 'extreme_direction'
]
REQUIRED_PROXIES = [
    'proxy_blue_ccpd2019_real.csv', 'proxy_blue_crpd_real.csv', 'proxy_green_ccpd2020_real.csv',
    'proxy_green_nonanhui_template_synth.csv', 'proxy_green_bridge_exactquad.csv',
    'proxy_green_edgefit_hard.csv', 'proxy_green_edgefit_extreme.csv', 'proxy_support_cblprd.csv'
]
...
def main():
    ...
    for r in train:
        if r.get('source') == 'green_edgefit_extreme':
            ...
            train_mapping.append({
                'split': 'train', 'province': r['text'][0], 'tier': rec['tier'], 'direction': rec['direction'],
                'old_text': r['text'], 'new_text': rec['text'], 'old_img_path': r['img_path'], 'new_img_path': nr['img_path']
            })
        else:
    ...
    write_csv(OUT_DIR / 'train_B1A_E2_perspective_both_tightrear_eval_original.csv', out_train, fields)
    write_csv(OUT_DIR / 'val_B1A_E2_original_eval.csv', out_val, fields)
    ...
    for r in old_proxy:
        ...
        proxy_mapping.append({
            'split': 'proxy', 'province': r['text'][0], 'tier': rec['tier'], 'direction': rec['direction'],
            'old_text': r['text'], 'new_text': rec['text'], 'old_img_path': r['img_path'], 'new_img_path': nr['img_path']
        })
    ...
    validate_rows(new_train_rows, 248)
    validate_rows(new_proxy_rows, 93)
    summary = {
        'experiment': 'StageB1A-E2_perspective_both_tightrear_eval_original_plus_new_proxy',
        'definition_note': 'Replace original StageB1A train green_edgefit_extreme rows with E2 perspective-both/tightrear ccpd_board rows. Old proxy remains unchanged for main benchmark; separate new proxy replaces only extreme proxy for secondary evaluation.',
        'dataset_root': str(DATA_ROOT),
        'train_manifest': str(OUT_DIR / 'train_B1A_E2_perspective_both_tightrear_eval_original.csv'),
        'val_manifest': str(OUT_DIR / 'val_B1A_E2_original_eval.csv'),
        ...
        'preprocess_assertion': 'all E2 train/proxy replacement rows have ccpd_board + full quad + obb_warp/letterbox/nn/gray3/bgr/quad_pad=0.0',
    }
    ...
    (OUT_DIR / 'summary_E2.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding='utf-8')
    (NEW_PROXY_DIR / 'summary_E2_new_proxy.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding='utf-8')
