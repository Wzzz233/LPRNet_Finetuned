...
SAVE_DIR="experiments/curriculum_gray3_stageB2_A2_softfreeze_backbone001"
LOG_PATH="$SAVE_DIR/train.log"
REPORT="reports/GREEN_STAGEB2_A2_SOFTFREEZE_BACKBONE001_REPORT.md"
SUMMARY="$SAVE_DIR/stageB2_A2_summary.json"
...
if ! grep -q "\[OptimGroup\] name=backbone .*lr_mult=0.01 .*lr=0.00000200" "$LOG_PATH"; then echo "[FATAL] backbone soft-freeze lr marker missing" >&2; exit 2; fi
...
report=root/'reports/GREEN_STAGEB2_A2_SOFTFREEZE_BACKBONE001_REPORT.md'
...
combined={'old_proxy':old,'new_v4e3_proxy':new,'baselines':baselines,'manifest_summary':summary,'qa_summary':qa,'definition':'StageB2-A2: A1D init + C train manifest + freeze_backbone False + freeze_bn_stats True + backbone_lr_mult 0.01 + head/adapter lr_mult 1.0'}
(save/'stageB2_A2_summary.json').write_text(json.dumps(combined, ensure_ascii=False, indent=2), encoding='utf-8')
def pct(x): return f'{x*100:.2f}%'
lines=[]
lines.append('# GREEN_STAGEB2_A2_SOFTFREEZE_BACKBONE001_REPORT')
lines.append('')
lines.append('日期: 2026-04-25')
lines.append('目的: 验证范式三 soft-freeze 第二参数点：A1D mother 上，backbone 极低 LR(0.01x) + head/adapter 正常 LR + BN frozen，使用 C 级别 300 条 v4_e3 ccpd_board extreme，不做 900 扩量。')
lines.append('')
lines.append('## 1. 实验设计')
...
lines.append('- 核心变量: freeze_backbone=False, freeze_bn_stats=True, backbone_lr_mult=0.01, head/adapter/aux lr_mult=1.0。')
...
lines.append('')
lines.append('## 4. 初步判读')
if old['real_avg'] >= 0.732 and old['green_ccpd'] >= 0.740 and old['blue_crpd'] >= 0.668 and old['hard'] >= 0.945 and old['extreme'] >= baselines['D_old']['extreme']:
    lines.append('- PASS 倾向：0.01x soft-freeze 守住 foundation 且不低于 D/B1A extreme，可进入 B2-B 采样权重实验。')
else:
    lines.append('- 未完全通过：0.01x 仍未同时守住 foundation 与 extreme；下一步应考虑只开放最后 backbone blocks 或回到 C hard-freeze 路线。')
