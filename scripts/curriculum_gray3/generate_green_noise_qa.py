#!/usr/bin/env python3
"""
Calibrate: GT quad + Gaussian noise → occ distribution.
Pick CCPD2020 green test images, add varying noise, measure occ.
Generate 60 QA samples at target occ range.
"""
...
# ── Consts ───────────────────────────────────────────────────────
QA_N = 60
OCC_TARGET_MIN, OCC_TARGET_MAX = 0.50, 0.85
NOISE_SIGMA = 4.0  # pixels, calibrated to produce occ ~0.5-0.85

OUT_ROOT = ROOT / 'datasets/ccpd2020_green_noise_qa'
QA_DIR = OUT_ROOT / 'qa'
for d in [QA_DIR]:
...
def add_noise_to_quad(quad, sigma):
    """Add independent Gaussian noise to each of the 4 points (x,y)."""
    noise = np.random.randn(4, 2).astype(np.float32) * sigma
    noisy = quad.copy().reshape(4, 2) + noise
    return noisy.reshape(-1, 2)
...
# ═══════════════════════════════════════════════════════════════════
#  STEP 1: Load CCPD2020 green test images
# ═══════════════════════════════════════════════════════════════════
print("STEP 1: Loading CCPD2020 green test images...")
...
with open(label_path, encoding='utf-8-sig') as f:
    for row in csv.DictReader(f):
        ...
        if q is None:
            continue
        a = angle_score(q)
        candidates.append({'path': p, 'gt_quad': q, 'angle': a, 'text': row['text']})
print(f"  Total CCPD2020 green: {len(candidates)}")

# ═══════════════════════════════════════════════════════════════════
#  STEP 2: Calibrate noise → test occ on top-angle images
# ═══════════════════════════════════════════════════════════════════
print(f"\nSTEP 2: Calibrating noise sigma={NOISE_SIGMA} px...")
candidates.sort(key=lambda x: -x['angle'])
top400 = candidates[:400]
...
for s in top400:
    ...
    calibrated.append({**s, 'noisy_quad': noisy, 'occ': float(occ)})

in_range = [c for c in calibrated if OCC_TARGET_MIN <= c['occ'] <= OCC_TARGET_MAX]
print(f"  Calibrated on {len(calibrated)} images")
print(f"  Occ range: {min(c['occ'] for c in calibrated):.3f} – {max(c['occ'] for c in calibrated):.3f}")
print(f"  In occ target [{OCC_TARGET_MIN},{OCC_TARGET_MAX}]: {len(in_range)}/{len(calibrated)}")

# ═══════════════════════════════════════════════════════════════════
#  STEP 3: Sample from target range, generate replacements
# ═══════════════════════════════════════════════════════════════════
...
for i, s in enumerate(selected):
    ...
    # Compositing: expanded GT quad for clean replacement
    ...
    # 94x24 gray3: noisy quad (simulates imprecise board OBB)
    orig_94, orig_occ = board_94x24(img, noisy_quad, 'gray3')
...
draw.text((10, 8), f"CCPD2020 Green + GT quad + σ={NOISE_SIGMA}px noise — QA Preview", fill=(220,220,220), font=tf)
headers = ['Original Photo', 'Noisy 94x24 gray3', 'Replaced Photo', 'New 94x24 gray3']
...
for ri, it in enumerate(items):
    ...
    lbl = f"occ={it['occ']:.2f} angle={it['angle']:.0f} σ={NOISE_SIGMA}px"
...
# ── Copy to Windows ──────────────────────────────────────────────
win_dir = Path('/mnt/c/Users/Wzzz2/OneDrive/Desktop/QA/ccpd2020_green_noise_qa')
