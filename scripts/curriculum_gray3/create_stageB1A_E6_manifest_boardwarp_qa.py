#!/usr/bin/env python3
"""QA final StageB1A-E6 manifests using actual ccpd_board final94 path."""
...
E6_DIR = ROOT / 'manifests/curriculum_gray3_stageb_v1_B1A_E6_axis_dominant_perspective_eval_original'
NEW_PROXY_DIR = ROOT / 'manifests/curriculum_gray3_stageb_v1_B1A_E6_axis_dominant_perspective_new_proxy'
OUT = ROOT / 'reports/stageB1A_E6_axis_dominant_perspective_manifest_QA_20260427'
WIN = Path('/mnt/c/Users/Wzzz2/OneDrive/Desktop/QA/stageB1A_E6_axis_dominant_perspective_manifest_QA_20260427')
...
def pick_by_prov(rows, n=48):
    by = defaultdict(list)
    for r in rows:
        by[(r.get('text') or '')[:1]].append(r)
    for k in by:
        by[k].sort(key=lambda r: (r.get('difficulty_tier', ''), r.get('extreme_direction', ''), abs_path(r).name))
    out = []
...
def render_rows(title, subtitle, rows, out_path, mode):
    ...
    d.text((10, 54), f'font={FONT_PATH}', font=TINY, fill=(90, 90, 90))
    recs = []
    for i, r in enumerate(rows):
        ...
        direction = r.get('extreme_direction', '')
        d.text((x + 8, y + 208), f'{i:02d} {txt} {tier}/{direction} {r.get("source","")[:26]}', font=TINY, fill=(0, 0, 0))
        d.text((x + 8, y + 224), f'{p.parent.name}/{p.name[:46]}', font=TINY, fill=(0, 0, 120))
        if qm:
            d.text((x + 8, y + 240), f"angle={qm['angle_score']:.1f} ratio={qm['ratio']:.2f} area={qm['area']:.0f}", font=TINY, fill=(80, 0, 100))
        ...
        if bb:
            rec.update({k: v for k, v in bb.items() if k != 'bbox'})
            rec['bbox'] = bb['bbox']
            color = (180, 0, 0) if bb['margin_l'] < 0.02 or bb['margin_r'] < 0.02 or bb['margin_t'] < 0.02 or bb['margin_b'] < 0.02 else (0, 120, 0)
            d.text((x + 178, y + 208), f"bbox={bb['bbox']} ow/oh={bb['occ_w']:.2f}/{bb['occ_h']:.2f}", font=TINY, fill=color)
            d.text((x + 178, y + 224), f"mLRTB={bb['margin_l']:.2f},{bb['margin_r']:.2f},{bb['margin_t']:.2f},{bb['margin_b']:.2f}", font=TINY, fill=color)
        recs.append(rec)
...
def quad_direction_stats(rows):
    ...
    for r in rows:
        q = get_quad(r)
        if q is None:
            stats['missing_quad'] += 1
            continue
        left = (q[0] + q[3]) / 2
        right = (q[1] + q[2]) / 2
        top = (q[0] + q[1]) / 2
        bot = (q[3] + q[2]) / 2
        rise = float(right[1] - left[1])
        skew = float(bot[0] - top[0])
...
def summarize_numeric(records):
    ...
    for k in ['occ','cx_norm','cy_norm','occ_w','occ_h','margin_l','margin_r','margin_t','margin_b','quad_angle_score','quad_ratio','quad_area','quad_min_edge']:
        ...
        if xs:
            ...
            out[k] = {'mean': float(arr.mean()), 'p50': float(np.percentile(arr, 50)), 'p05': float(np.percentile(arr, 5)), 'p95': float(np.percentile(arr, 95)), 'min': float(arr.min()), 'max': float(arr.max())}
    return out
...
def assert_manifest(rows, n, source):
    ...
    for r in rows:
        if r.get('source') != source:
            bad.append(('source', r.get('source')))
        ...
        for k in QKEYS:
            if r.get(k, '') == '':
                bad.append((k, ''))
        if not abs_path(r).exists():
            bad.append(('missing', str(abs_path(r))))
...
def main():
    ...
    e6_train = [r for r in read_rows(E6_DIR / 'train_B1A_E6_axis_dominant_perspective_eval_original.csv') if r.get('source') == NEW_SOURCE]
    old_proxy = read_rows(E6_DIR / 'proxy_green_edgefit_extreme.csv')
    new_proxy = [r for r in read_rows(NEW_PROXY_DIR / 'proxy_green_edgefit_extreme.csv') if r.get('source') == NEW_SOURCE]
    assert_manifest(e6_train, 300, NEW_SOURCE)
    assert_manifest(new_proxy, 124, NEW_SOURCE)

    recs = []
    recs += render_rows('OLD B1A train extreme: baseline tier3 preview', '每格: 原图+quad / warp / final94 gray3；用于对比原 extreme 样本。', pick_by_prov(orig_train, 48), OUT / '01_old_B1A_train_extreme_boardwarp.jpg', 'filename_quad')
    recs += render_rows('E6 train LOW: axis-dominant perspective', '单轴偏斜为主，少量复合偏斜补充；强调更真实的近矩形四边形。', pick_by_prov([r for r in e6_train if r.get('difficulty_tier') == 'low'], 48), OUT / '02_E6_train_low_final94.jpg', 'manifest_quad')
    recs += render_rows('E6 train MID: axis-dominant perspective', '单轴偏斜为主，少量复合偏斜补充；强调更真实的近矩形四边形。', pick_by_prov([r for r in e6_train if r.get('difficulty_tier') == 'mid'], 48), OUT / '03_E6_train_mid_final94.jpg', 'manifest_quad')
    recs += render_rows('E6 train HIGH: axis-dominant perspective', '单轴偏斜为主，少量复合偏斜补充；强调更真实的近矩形四边形。', pick_by_prov([r for r in e6_train if r.get('difficulty_tier') == 'high'], 48), OUT / '04_E6_train_high_final94.jpg', 'manifest_quad')
    recs += render_rows('E6 old proxy unchanged: original benchmark', '主评测继续使用原 proxy，便于和旧实验直接对比。', pick_by_prov(old_proxy, 48), OUT / '05_old_proxy_unchanged_preview.jpg', 'filename_quad')
    recs += render_rows('E6 new proxy LOW/MID/HIGH: secondary eval', '新 proxy 使用同版 axis-dominant 几何，供辅助评测。', pick_by_prov(new_proxy, 60), OUT / '06_new_proxy_lmh_final94.jpg', 'manifest_quad')

    train_paths = {str(abs_path(r)) for r in e6_train}
    proxy_paths = {str(abs_path(r)) for r in new_proxy}
    summary = {
        'out_dir': str(OUT),
        'windows_out_dir': str(WIN),
        'inputs': {'orig_train_extreme_count': len(orig_train), 'e6_train_extreme_count': len(e6_train), 'old_proxy_count': len(old_proxy), 'new_proxy_count': len(new_proxy), 'train_new_proxy_overlap': len(train_paths & proxy_paths)},
        'province_counts': {'orig_train': dict(sorted(Counter(r['text'][0] for r in orig_train).items())), 'e6_train': dict(sorted(Counter(r['text'][0] for r in e6_train).items())), 'old_proxy': dict(sorted(Counter(r['text'][0] for r in old_proxy).items())), 'new_proxy': dict(sorted(Counter(r['text'][0] for r in new_proxy).items()))},
        'tier_counts': {'e6_train': dict(sorted(Counter(r.get('difficulty_tier', '') for r in e6_train).items())), 'new_proxy': dict(sorted(Counter(r.get('difficulty_tier', '') for r in new_proxy).items()))},
        'direction_counts': {'e6_train': dict(sorted(Counter(r.get('extreme_direction', '') for r in e6_train).items())), 'new_proxy': dict(sorted(Counter(r.get('extreme_direction', '') for r in new_proxy).items()))},
        'direction_stats_e6_train': quad_direction_stats(e6_train),
        'direction_stats_new_proxy': quad_direction_stats(new_proxy),
        'sample_numeric_summary': summarize_numeric(recs),
        'sheets': [str(p) for p in sorted(OUT.glob('*.jpg'))],
        'precheck': 'manifest rows assert ccpd_board + full quad + obb_warp/letterbox/nn/gray3/bgr/quad_pad=0.0; train/proxy overlap=0',
    }
    (OUT / 'summary.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding='utf-8')
    with (OUT / 'sample_records.csv').open('w', encoding='utf-8', newline='') as f:
        fields = sorted({k for r in recs for k in r.keys()})
