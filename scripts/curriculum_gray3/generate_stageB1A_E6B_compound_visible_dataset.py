#!/usr/bin/env python3
"""Generate StageB1A-E6B compound visible perspective dataset.

Only compound directions are used: left_up/left_down/right_up/right_down.
Current E6 high tilt is used as the new mid reference, then expanded to low/high.
"""
...
OUT = ROOT / 'tmp/green_extreme_stageB1A_E6B_preblur_v3_20260428'
PLATE_MIN = (470, 120)
PLATE_MAX_W = 640
PLATE_MAX_H = 192
PNG_COMPRESSION = 3
...
TIER_PARAMS = {
    'low': dict(ratio=(2.04, 2.38), height=(84, 102), yaw_side=(0.82, 0.92), pitch=(0.84, 1.04), shear=(3.0, 6.0), roll=(-3.0, 3.0), min_area=11600, min_edge=46, min_angle_gap=6.0, max_angle_gap=17),
    'mid': dict(ratio=(1.96, 2.34), height=(84, 102), yaw_side=(0.76, 0.88), pitch=(0.80, 1.08), shear=(4.5, 9.0), roll=(-4.0, 4.0), min_area=11000, min_edge=43, min_angle_gap=10.0, max_angle_gap=22),
    'high': dict(ratio=(1.88, 2.28), height=(84, 100), yaw_side=(0.66, 0.82), pitch=(0.72, 1.12), shear=(6.5, 12.0), roll=(-5.0, 5.0), min_area=10200, min_edge=40, min_angle_gap=14.0, max_angle_gap=30),
}
DIRECTION_GROUPS = {
    'left_up': ('left', 'up'),
    'left_down': ('left', 'down'),
...
YAW_MIX = {'low': 0.34, 'mid': 0.52, 'high': 0.74}
PITCH_MIX = {'low': 0.34, 'mid': 0.52, 'high': 0.74}
...
def target_quad(rng, direction, tier):
    p = TIER_PARAMS[tier]
    ...
    yaw_near = rng.uniform(1.00, 1.10)
    yaw_mix = YAW_MIX[tier]
    pitch_low, pitch_high = p['pitch']
    pitch_mix = PITCH_MIX[tier]
    ...
    br = np.array([cx + bottom_w / 2, cy + right_h / 2 - shear * 0.32], np.float32)
    bl = np.array([cx - bottom_w / 2, cy + left_h / 2 + shear * 0.32], np.float32)
...
def make_one(rng, stats, bgs, labels_by_prov, prov, tier, direction, split, idx):
    ...
    while attempts < 5000:
        ...
        if img is None or srcq is None:
            continue
        q = target_quad(rng, direction, tier)
        if not gate(q, tier):
            continue
        bg = cv2.resize(rng.choice(bgs), CANVAS)
        # Keep the new E6B geometry, but render through a plate-sized intermediate.
        ...
        fname = f'E6Bcmp-{bbox}-{qstr}-{split}-{tier}-{direction}-{prov}-{idx:04d}-{text}.png'
