...
MANIFEST_DIR="manifests/curriculum_gray3_stageb_v1_B1A_D_extreme900_v4e3_ccpdboard_eval_original"
NEW_PROXY_MANIFEST_DIR="manifests/curriculum_gray3_stageb_v1_B1A_D_new_v4e3_ccpdboard_proxy"
TRAIN_MANIFEST="$MANIFEST_DIR/train_B1A_D_extreme900_v4e3_ccpdboard_eval_original.csv"
VAL_MANIFEST="$MANIFEST_DIR/val_B1A_D_original_eval.csv"
SAVE_DIR="experiments/curriculum_gray3_stageB_v1_B1A_D_extreme900_v4e3_ccpdboard_eval_original"
LOG_PATH="$SAVE_DIR/train.log"
REPORT="reports/GREEN_STAGEB1A_D_EXTREME900_V4E3_CCPDBOARD_REPORT.md"
...
python3 scripts/curriculum_gray3/build_stageB1A_D_extreme900_v4e3_ccpdboard_eval_original_plus_new_proxy.py > /tmp/stageB1A_D_manifest_build.json
python3 scripts/curriculum_gray3/create_stageB1A_D_manifest_boardwarp_qa.py > /tmp/stageB1A_D_manifest_qa.json

python3 - <<'PY'
import csv, json, sys
from pathlib import Path
root=Path('/home/wzzz/LPRNet')
mdir=root/'manifests/curriculum_gray3_stageb_v1_B1A_D_extreme900_v4e3_ccpdboard_eval_original'
nmdir=root/'manifests/curriculum_gray3_stageb_v1_B1A_D_new_v4e3_ccpdboard_proxy'
...
summary=json.loads((mdir/'summary_D.json').read_text(encoding='utf-8'))
qa=json.loads((root/'reports/stageB1A_D_extreme900_v4e3_ccpdboard_manifest_QA_20260425/summary.json').read_text(encoding='utf-8'))
if summary['train_extreme_count'] != 900 or summary['train_new_proxy_path_overlap_count'] != 0:
    print('[FATAL] bad summary '+json.dumps(summary, ensure_ascii=False), file=sys.stderr); sys.exit(2)
if summary['train_rows'] != 66175 or summary['train_row_delta'] != 600:
    print('[FATAL] row count mismatch '+json.dumps({'rows':summary['train_rows'],'delta':summary['train_row_delta']}, ensure_ascii=False), file=sys.stderr); sys.exit(2)
...
train=mdir/'train_B1A_D_extreme900_v4e3_ccpdboard_eval_original.csv'
count=0; bad=[]
with train.open(encoding='utf-8', newline='') as f:
    for row in csv.DictReader(f):
        if row.get('source')=='green_edgefit_extreme900_v4e3_ccpdboard':
            count+=1
...
if bad:
    print('[FATAL] bad D extreme rows '+repr(bad[:10]), file=sys.stderr); sys.exit(2)
print(json.dumps({'preflight':'ok','train_extreme_count':count,'train_rows':summary['train_rows'],'qa_dir':qa['windows_out_dir']}, ensure_ascii=False))
PY
...
from pathlib import Path
root=Path('/home/wzzz/LPRNet')
save=root/'experiments/curriculum_gray3_stageB_v1_B1A_D_extreme900_v4e3_ccpdboard_eval_original'
report=root/'reports/GREEN_STAGEB1A_D_EXTREME900_V4E3_CCPDBOARD_REPORT.md'
summary=json.loads((root/'manifests/curriculum_gray3_stageb_v1_B1A_D_extreme900_v4e3_ccpdboard_eval_original/summary_D.json').read_text(encoding='utf-8'))
qa=json.loads((root/'reports/stageB1A_D_extreme900_v4e3_ccpdboard_manifest_QA_20260425/summary.json').read_text(encoding='utf-8'))
...
old=top(save/'proxy_eval_stageB_v1_old_proxy'); new=top(save/'proxy_eval_stageB_v1_new_v4e3_proxy')
combined={'old_proxy':old,'new_v4e3_proxy':new,'manifest_summary':summary,'qa_summary':qa}
(save/'stageB1A_D_summary.json').write_text(json.dumps(combined, ensure_ascii=False, indent=2), encoding='utf-8')
def pct(x): return f'{x*100:.2f}%'
lines=[]
lines.append('# GREEN_STAGEB1A_D_EXTREME900_V4E3_CCPDBOARD_REPORT')
lines.append('')
lines.append('日期: 2026-04-25')
lines.append('目的: StageB1A-D，将 train extreme 从 300 扩到 900，使用用户确认的 v4_e3 中等倾斜 extreme，训练走 quad/ccpd_board；old proxy 保持原 benchmark，new v4_e3 proxy 只作新增难度评测。')
lines.append('')
lines.append('## 1. 实验设计')
...
lines.append(f"- train rows: {summary['train_rows']}，相对原 B1A +{summary['train_row_delta']}")
lines.append('- 移除原 source=green_edgefit_extreme 的 300 行，追加 900 行 v4_e3 ccpd_board extreme；非 extreme 行保持不变。')
lines.append('- train extreme 各省计数为原 B1A 的 3 倍；old proxy path+text 完全不变；new proxy 与 train 新样本路径 0 重叠。')
...
for s in qa['sheets']: lines.append(f"- QA 图: {s}")
lines.append(f"- train/proxy overlap: {qa['inputs']['train_new_proxy_overlap']}")
lines.append(f"- D train direction stats: {json.dumps(qa['direction_stats_d_train'], ensure_ascii=False)}")
...
lines.append('')
lines.append('## 4. 解释口径')
lines.append('- old_proxy 是主结论口径，可与原 StageB1A old proxy 和 C 轮横比。')
lines.append('- new_v4e3_proxy 是新增难度口径，只能与同一 new proxy 上的历史 B/C/D 结果比较。')
lines.append('- 本实验改变了 train extreme 注入量 300->900，同时保持 old proxy 不变；它回答“扩大 v4_e3 ccpd_board extreme 注入是否改善新/旧 extreme”。')
report.write_text('\n'.join(lines)+'\n', encoding='utf-8')
print('\n'.join(lines))
PY
