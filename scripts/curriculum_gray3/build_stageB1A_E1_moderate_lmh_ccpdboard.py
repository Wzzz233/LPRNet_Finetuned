#!/usr/bin/env python3
"""Build StageB1A-E1 manifests from accepted moderate low/mid/high extreme data.

Definition:
- Start from original StageB1A difficulty manifests.
- Replace only train source=green_edgefit_extreme rows (300) with E1 moderate LMH ccpd_board rows.
- Keep original validation/proxy files unchanged for main old benchmark.
- Produce a separate new-proxy manifest dir replacing only proxy_green_edgefit_extreme with non-overlapping E1 proxy rows.
"""
...
DATA_ROOT=ROOT/'tmp/green_extreme_stageB1A_E1_moderate_lmh_20260426'
OUT_DIR=ROOT/'manifests/curriculum_gray3_stageb_v1_B1A_E1_moderate_lmh_ccpdboard_eval_original'
...
def fill_e1_row(base, rec, fields):
    ...
    rr.update({
        ...
        'source': 'green_edgefit_extreme_E1_moderate_lmh_ccpdboard',
        'source_family': 'green_edgefit_extreme_E1_moderate_lmh_ccpdboard__green8',
...
def validate_e1_rows(rows, expected_count):
    bad=[]
    for r in rows:
    ...
    if len(rows)!=expected_count or bad:
        raise SystemExit(json.dumps({'fatal':'validate_e1_rows','count':len(rows),'expected':expected_count,'bad':bad[:20]},ensure_ascii=False,indent=2))

def main():
    train,base_fields=read_rows(ORIG_TRAIN); val,_=read_rows(ORIG_VAL)
    ...
    write_csv(OUT_DIR/'train_B1A_E1_moderate_lmh_ccpdboard_eval_original.csv',out_train,fields)
    write_csv(OUT_DIR/'val_B1A_E1_original_eval.csv',out_val,fields)
    ...
    non_extreme_new=[r for r in out_train if r.get('source')!='green_edgefit_extreme_E1_moderate_lmh_ccpdboard']
    ...
    train_paths={m['new_img_path'] for m in train_mapping}; proxy_paths={m['new_img_path'] for m in proxy_mapping}
    overlap=sorted(train_paths & proxy_paths)
    validate_e1_rows(new_train_rows,300); validate_e1_rows(new_proxy_rows,124)
    summary={
        'experiment':'StageB1A-E1_moderate_lmh_ccpdboard_eval_original_plus_new_proxy',
        'definition_note':'Only original StageB1A train green_edgefit_extreme rows are replaced with accepted moderate-angle low/mid/high ccpd_board rows. Old proxy remains unchanged for main benchmark; separate new proxy replaces only extreme proxy for secondary evaluation.',
        'dataset_root':str(DATA_ROOT),
        'train_manifest':str(OUT_DIR/'train_B1A_E1_moderate_lmh_ccpdboard_eval_original.csv'),
        'val_manifest':str(OUT_DIR/'val_B1A_E1_original_eval.csv'),
        ...
        'preprocess_assertion':'all E1 train/proxy replacement rows have ccpd_board + full quad + obb_warp/letterbox/nn/gray3/bgr/quad_pad=0.0',
    }
    ...
    (OUT_DIR/'summary_E1.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
    (NEW_PROXY_DIR/'summary_E1_new_proxy.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
