#!/usr/bin/env python3
"""QA final StageB1A-E2 train/proxy manifests using actual ccpd_board final94 path."""
import csv
import json
import math
import shutil
...
E2_DIR = ROOT / 'manifests/curriculum_gray3_stageb_v1_B1A_E2_perspective_both_tightrear_eval_original'
NEW_PROXY_DIR = ROOT / 'manifests/curriculum_gray3_stageb_v1_B1A_E2_perspective_both_tightrear_new_proxy'
OUT = ROOT / 'reports/stageB1A_E2_perspective_both_tightrear_manifest_QA_20260427'
WIN = Path('/mnt/c/Users/Wzzz2/OneDrive/Desktop/QA/stageB1A_E2_perspective_both_tightrear_manifest_QA_20260427')
...
NEW_SOURCE = 'green_edgefit_extreme_E2_perspective_both_tightrear_ccpdboard'
...
def render_rows(title, rows, out_path, mode):
    ...
    d.text((10, 34), '每格: 原图+quad / warp / final94 gray3；E2强调双向透视+后段压缩。', font=SMALL, fill=(50, 50, 50))
    ...
    for i, r in enumerate(rows):
        ...
        d.text((x + 8, y + 208), f'{i:02d} {txt} {tier}/{direction} {r.get("source","")[:24]}', font=TINY, fill=(0, 0, 0))
...
def main():
    ...
    e2_train = [r for r in read_rows(E2_DIR / 'train_B1A_E2_perspective_both_tightrear_eval_original.csv') if r.get('source') == NEW_SOURCE]
    old_proxy = read_rows(E2_DIR / 'proxy_green_edgefit_extreme.csv')
    new_proxy = [r for r in read_rows(NEW_PROXY_DIR / 'proxy_green_edgefit_extreme.csv') if r.get('source') == NEW_SOURCE]
    assert_manifest(e2_train, 248, NEW_SOURCE)
    assert_manifest(new_proxy, 93, NEW_SOURCE)
    recs = []
    recs += render_rows('OLD B1A train extreme: baseline tier3 black-border preview', pick_by_prov(orig_train, 48), OUT / '01_old_B1A_train_extreme_boardwarp.jpg', 'filename_quad')
    recs += render_rows('E2 train MID: both-tilt perspective tightrear final training input', pick_by_prov([r for r in e2_train if r.get('difficulty_tier') == 'mid'], 48), OUT / '02_E2_train_mid_final94.jpg', 'manifest_quad')
    recs += render_rows('E2 train HIGH: both-tilt perspective tightrear final training input', pick_by_prov([r for r in e2_train if r.get('difficulty_tier') == 'high'], 48), OUT / '03_E2_train_high_final94.jpg', 'manifest_quad')
    recs += render_rows('E2 old proxy unchanged: original StageB1A benchmark', pick_by_prov(old_proxy, 48), OUT / '04_old_proxy_unchanged_preview.jpg', 'filename_quad')
    recs += render_rows('E2 new proxy MID/HIGH: secondary accepted-domain eval', pick_by_prov(new_proxy, 60), OUT / '05_new_proxy_midhigh_final94.jpg', 'manifest_quad')
    train_paths = {str(abs_path(r)) for r in e2_train}
    proxy_paths = {str(abs_path(r)) for r in new_proxy}
    summary = {
        'out_dir': str(OUT),
        'windows_out_dir': str(WIN),
        'inputs': {
            'orig_train_extreme_count': len(orig_train),
            'e2_train_extreme_count': len(e2_train),
            'old_proxy_count': len(old_proxy),
            'new_proxy_count': len(new_proxy),
            'train_new_proxy_overlap': len(train_paths & proxy_paths),
        },
        'province_counts': {
            'orig_train': dict(sorted(Counter(r['text'][0] for r in orig_train).items())),
            'e2_train': dict(sorted(Counter(r['text'][0] for r in e2_train).items())),
            'old_proxy': dict(sorted(Counter(r['text'][0] for r in old_proxy).items())),
            'new_proxy': dict(sorted(Counter(r['text'][0] for r in new_proxy).items())),
        },
        'tier_counts': {
            'e2_train': dict(sorted(Counter(r.get('difficulty_tier', '') for r in e2_train).items())),
            'new_proxy': dict(sorted(Counter(r.get('difficulty_tier', '') for r in new_proxy).items())),
        },
        'direction_counts': {
            'e2_train': dict(sorted(Counter(r.get('extreme_direction', '') for r in e2_train).items())),
            'new_proxy': dict(sorted(Counter(r.get('extreme_direction', '') for r in new_proxy).items())),
        },
        'direction_stats_e2_train': quad_direction_stats(e2_train),
