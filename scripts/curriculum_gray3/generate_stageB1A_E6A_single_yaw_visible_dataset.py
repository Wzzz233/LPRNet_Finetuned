#!/usr/bin/env python3
"""Generate StageB1A-E6A single-yaw visible dataset.

Directions: left/right only.
This isolates horizontal perspective so pure left/right can be tuned independently.
Half-quota version: this script owns the yaw half of the final E6A pool.
"""
...
OUT = ROOT / 'tmp/green_extreme_stageB1A_E6A_single_yaw_visible_20260427'
PLATE = (470, 120)
CANVAS = (920, 600)
TIERS = ['low', 'mid', 'high']
...
TIER_PARAMS = {
    'low': dict(ratio=(2.08, 2.44), height=(84, 102), yaw_side=(0.84, 0.94), shear=(1.8, 4.0), roll=(-2.2, 2.2), min_area=11600, min_edge=46, min_angle_gap=4.0, max_angle_gap=12),
    'mid': dict(ratio=(2.04, 2.42), height=(84, 102), yaw_side=(0.76, 0.88), shear=(2.8, 5.5), roll=(-2.8, 2.8), min_area=11000, min_edge=43, min_angle_gap=6.5, max_angle_gap=16),
    'high': dict(ratio=(1.96, 2.36), height=(84, 100), yaw_side=(0.66, 0.82), shear=(4.0, 7.5), roll=(-3.6, 3.6), min_area=10400, min_edge=41, min_angle_gap=10.0, max_angle_gap=22),
}
YAW_MIX = {'low': 0.30, 'mid': 0.50, 'high': 0.72}
...
def target_quad(rng, direction, tier, relax=1.0):
    ...
    br = np.array([cx + bottom_w / 2, cy + right_h / 2 - shear * 0.22], np.float32)
    bl = np.array([cx - bottom_w / 2, cy + left_h / 2 + shear * 0.22], np.float32)
...
def make_one(rng, stats, bgs, labels_by_prov, prov, tier, direction, split, idx):
    ...
    while attempts < 5000:
        ...
        fname = f'E6Ayaw-{bbox}-{qstr}-{split}-{tier}-{direction}-{prov}-{idx:04d}-{text}.jpg'
...
def main():
    ...
    train_quota = split_half_quota(FULL_TRAIN_QUOTA, take_upper_half=False)
    proxy_quota = split_half_quota(FULL_PROXY_QUOTA, take_upper_half=False)
