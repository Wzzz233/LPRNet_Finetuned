#!/usr/bin/env python3
"""Re-scan all 6 ratio points using the CORRECT original protocol.
   Produces consistent comparison across all points.
"""
...
EXPS = {
    'R00_replace_only': ROOT / 'experiments/a_ablation_replace_only_20260510',
    'R06_mix_rebuild': ROOT / 'experiments/a_ablation_mix_rebuild_20260510',
    'R10': ROOT / 'experiments/a_ratio_r10_20260510',
    'R20': ROOT / 'experiments/a_ratio_r20_20260510',
    'R35': ROOT / 'experiments/a_ratio_r35_20260510',
    'R50': ROOT / 'experiments/a_ratio_r50_20260510',
    'R100_real_only': ROOT / 'experiments/a_ablation_real_only_20260510',
}
...
for label, exp_dir in EXPS.items():
    ...
    for ckpt_name in CKPTS:
        ...
        exp_data[ckpt_name] = {'pos_ocr_dump': d1, 'static_control': d2}
    ...
    all_results[label] = exp_data

# ── Board-optimal selection ──
def pick_optimal(exp_data):
    cands = [(ckpt, d['pos_ocr_dump']['seg_B']['pp_char'],
             ...
             for ckpt, d in exp_data.items()]
    cands.sort(key=lambda x: (-x[1], x[2]))
    return cands[0] if cands else None

print(f'\n{"="*70}')
print('BOARD-OPTIMAL PER BRANCH (consistent protocol)')
print(f'{"="*70}')
for label in EXPS:
    best = pick_optimal(all_results[label])
    if best:
        ckpt, pp_char, len_err, pp_ex, st_pp_ex, prov = best
        bias = ' '.join(f'{k}={v}' for k,v in list(prov.items())[:3]) if prov else 'none'
        print(f'{label:20s} winner={ckpt:30s} seg_pp_char={pp_char:.1f}% len_err={len_err:.1f}% '
              f'static_pp_ex={st_pp_ex:.1f}% bias={bias}')

# ── Save per-experiment ──
for label in EXPS:
    out_path = OUT_DIR / f'recheck_ratio_{label}.json'
    json.dump(all_results[label], open(out_path, 'w'), ensure_ascii=False, indent=2)
    print(f'Saved: {out_path}')

# ── Comparison table ──
print(f'\n{"="*70}')
print('COMPARISON TABLE (correct protocol)')
print(f'{"="*70}')
hdr = f'{"Branch":20s} {"Winner":30s} {"pp_char":>8s} {"len_err":>8s} {"pp_exact":>8s} {"st_pp_ex":>8s} {"st_pp_char":>8s}'
print(hdr)
print('-' * 100)
for label in EXPS:
    best = pick_optimal(all_results[label])
    ...
    ckpt, pp_char, len_err, pp_ex, st_pp_ex, prov = best
    print(f'{label:20s} {ckpt:30s} {pp_char:>7.1f}% {len_err:>7.1f}% {pp_ex:>7.1f}% {st_pp_ex:>7.1f}% {"":>8s}')
