...
TRAIN_MANIFEST=/home/wzzz/LPRNet/manifests/unified_manifest_green_e20a_cluster2_beijing_prefix_contrast_1200.csv
...
SAVE_DIR=/home/wzzz/LPRNet/experiments/green_e24a_cluster2_green8_adapter_pos0_e20adata_stage2
...
for f in "$INIT" "$TRAIN_MANIFEST" "$EVAL_MANIFEST" "$E7_MANIFEST" "$CLUSTER2_CSV" "$CLUSTER3_CSV" "$CLUSTER3_TAIL_CSV"; do
...
echo "[RUN] E24A cluster2 green8-adapter + green8-only pos0 on E20A contrast data"
echo "[INIT] $INIT"
...
echo "[EVAL] cluster2 dump replay (ctc-only)"
...
echo "[EVAL] cluster2 dump replay (hybrid pos0)"
$PY src/utils/replay_dump_compare.py \
  --model "$SAVE_DIR/Final_LPRNet_model.pth" \
  --input-csv "$CLUSTER2_CSV" \
  --out-json "$SAVE_DIR/eval_cluster2_dump_hybrid.json" \
  --out-csv "$SAVE_DIR/eval_cluster2_dump_hybrid.csv" \
  --pos0-fusion-mode replace_if_confident \
  --pos0-conf-threshold 0.55 \
  2>&1 | tee "$SAVE_DIR/eval_cluster2_dump_hybrid.log"

echo "[EVAL] cluster3 dump replay (ctc-only)"
$PY src/utils/replay_dump_compare.py \
  --model "$SAVE_DIR/Final_LPRNet_model.pth" \
...
echo "[EVAL] cluster3 dump replay (hybrid pos0)"
$PY src/utils/replay_dump_compare.py \
  --model "$SAVE_DIR/Final_LPRNet_model.pth" \
  --input-csv "$CLUSTER3_CSV" \
  --out-json "$SAVE_DIR/eval_cluster3_dump_hybrid.json" \
  --out-csv "$SAVE_DIR/eval_cluster3_dump_hybrid.csv" \
  --pos0-fusion-mode replace_if_confident \
  --pos0-conf-threshold 0.55 \
  2>&1 | tee "$SAVE_DIR/eval_cluster3_dump_hybrid.log"

echo "[EVAL] cluster3 tail dump replay (ctc-only)"
$PY src/utils/replay_dump_compare.py \
  --model "$SAVE_DIR/Final_LPRNet_model.pth" \
...
echo "[EVAL] cluster3 tail dump replay (hybrid pos0)"
$PY src/utils/replay_dump_compare.py \
  --model "$SAVE_DIR/Final_LPRNet_model.pth" \
  --input-csv "$CLUSTER3_TAIL_CSV" \
  --out-json "$SAVE_DIR/eval_cluster3_tail_dump_hybrid.json" \
  --out-csv "$SAVE_DIR/eval_cluster3_tail_dump_hybrid.csv" \
  --pos0-fusion-mode replace_if_confident \
  --pos0-conf-threshold 0.55 \
  2>&1 | tee "$SAVE_DIR/eval_cluster3_tail_dump_hybrid.log"

echo "[STATUS] refresh shared status"
$PY src/utils/update_training_status.py \
  --log "$SAVE_DIR/train.log" \
  --process-keyword green_e24a_cluster2_green8_adapter_pos0_e20adata_stage2 \
  --final-weight "$SAVE_DIR/Final_LPRNet_model.pth" \
  --eval-json "$SAVE_DIR/eval_family_aware.json" \
  --out-json "$STATUS_JSON" \
  --out-md "$STATUS_MD"

echo "[DONE] E24A complete"
