...
SAVE_DIR=/home/wzzz/LPRNet/experiments/green_h32/H32C_classbalanced_beta9995_plus_focalctc
BASELINE_A=/home/wzzz/LPRNet/experiments/green_h32/H32A_classbalanced_firstchar/eval_green8_metrics_only.json
BASELINE_B=/home/wzzz/LPRNet/experiments/green_h32/H32B_classbalanced_plus_focalctc/eval_green8_metrics_only.json

mkdir -p /home/wzzz/LPRNet/experiments/green_h32
rm -rf "$SAVE_DIR"
mkdir -p "$SAVE_DIR"

case "proxy_exact" in
  proxy_exact|balanced_tuple|balanced_recovery) ;;
  *) echo "[FATAL] invalid selection strategy"; exit 2 ;;
esac
case "stratified" in
  sequential|stratified) ;;
  *) echo "[FATAL] invalid selection proxy mode"; exit 2 ;;
esac
case "class_balanced_ce" in
  ce|class_balanced_ce) ;;
  *) echo "[FATAL] invalid first_char_loss_type"; exit 2 ;;
esac
case "focal" in
  standard|focal) ;;
  *) echo "[FATAL] invalid ctc_loss_type"; exit 2 ;;
esac
if [ ! -f "$BASELINE_A" ] || [ ! -f "$BASELINE_B" ]; then
  echo "[FATAL] missing H32A/H32B baseline json"
  exit 3
fi

echo "[RUN] H32-C: H32B + tighter class-balanced first-char beta"
echo "[CONFIG] Keep H32B fixed; only first_char_cb_beta: 0.999 -> 0.9995"
echo "[TARGET] Try to recover 苏 first_char while preserving H32B overall gains; also inspect province-wise exact collapse-to-Anhui risk"
