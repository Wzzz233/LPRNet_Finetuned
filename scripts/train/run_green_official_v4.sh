...
usage() {
  cat <<'USAGE'
Usage:
  run_green_official_v4.sh [options]

Options:
  --run-tag <tag>
  --base-model <path>                             (default: ./experiments/green_official_v3_prod_20260320_190610/weights_stageA/Final_LPRNet_model.pth)
...
RUN_TAG="green_official_v4_prod_${DATE_TAG}"
BASE_MODEL="${ROOT_DIR}/experiments/green_official_v3_prod_20260320_190610/weights_stageA/Final_LPRNet_model.pth"
...
mkdir -p "${OUT_DIR}" "${LABELS_DIR}" "${GREEN_LABEL_SRC_DIR}" "${WEIGHTS_A}"
...
echo "[${step}] Build strict green splits (stageA-only v4)"
step=$((step + 1))
...
TEST_BAL_V4="${LABELS_DIR}/test_green_balanced_v2_labels.txt"
VAL_BAL_V4="${LABELS_DIR}/val_green_balanced_v2_labels.txt"
TRAIN_BAL_V4="${LABELS_DIR}/train_green_balanced_v2_labels.txt"
...
echo "[${step}] Baseline metrics from current best model"
step=$((step + 1))
run_eval "${TEST_BAL_V4}" "${BASE_MODEL}" "${OUT_DIR}/baseline_green_balanced_v4_test_metrics.json"
run_eval "${TEST_FULL}" "${BASE_MODEL}" "${OUT_DIR}/baseline_green_full_test_metrics.json"

echo "[${step}] Stage A short polish (balanced_v4 only)"
step=$((step + 1))
"${ENV_PY}" "${ROOT_DIR}/train_LPRNet.py" \
  "${TRAIN_COMMON_ARGS[@]}" \
  --train_txt_file "${TRAIN_BAL_V4}" \
  --test_txt_file "${VAL_BAL_V4}" \
...
step=$((step + 1))
run_eval "${TEST_BAL_V4}" "${CANDIDATE_MODEL}" "${OUT_DIR}/candidate_green_balanced_v4_metrics.json"
...
baseline_balanced = json.loads((run_dir / "baseline_green_balanced_v4_test_metrics.json").read_text(encoding="utf-8"))
baseline_full = json.loads((run_dir / "baseline_green_full_test_metrics.json").read_text(encoding="utf-8"))
candidate_balanced = json.loads((run_dir / "candidate_green_balanced_v4_metrics.json").read_text(encoding="utf-8"))
...
scorecard = {
    ...
    "baseline_green_balanced_metrics": {
        "exact_plate_acc": baseline_exact,
        "province_macro_exact_acc": float(baseline_balanced.get("province_macro_exact_acc", 0.0)),
        "province_macro_first_char_acc": float(baseline_balanced.get("province_macro_first_char_acc", 0.0)),
        "non_major_province_exact_acc": float(baseline_balanced.get("non_major_province_exact_acc", 0.0)),
        "major_province": baseline_balanced.get("major_province"),
        "major_province_ratio": float(baseline_balanced.get("major_province_ratio", 0.0)),
        "empty_pred_rate": baseline_empty,
    },
    "candidate_green_balanced_metrics": {
        "exact_plate_acc": candidate_exact,
        "province_macro_exact_acc": float(candidate_balanced.get("province_macro_exact_acc", 0.0)),
        "province_macro_first_char_acc": float(candidate_balanced.get("province_macro_first_char_acc", 0.0)),
        "non_major_province_exact_acc": float(candidate_balanced.get("non_major_province_exact_acc", 0.0)),
        "major_province": candidate_balanced.get("major_province"),
        "major_province_ratio": float(candidate_balanced.get("major_province_ratio", 0.0)),
        "empty_pred_rate": candidate_empty,
    },
    "adopted_green_balanced_metrics": {
        "exact_plate_acc": adopted_exact,
        "province_macro_exact_acc": float(adopted_balanced.get("province_macro_exact_acc", 0.0)),
        "province_macro_first_char_acc": float(adopted_balanced.get("province_macro_first_char_acc", 0.0)),
        "non_major_province_exact_acc": float(adopted_balanced.get("non_major_province_exact_acc", 0.0)),
        "major_province": adopted_balanced.get("major_province"),
        "major_province_ratio": float(adopted_balanced.get("major_province_ratio", 0.0)),
        "empty_pred_rate": adopted_empty,
    },
    "adopted_green_full_metrics": {
        "exact_plate_acc": adopted_full_exact,
        "province_macro_exact_acc": float(adopted_full.get("province_macro_exact_acc", 0.0)),
        "province_macro_first_char_acc": float(adopted_full.get("province_macro_first_char_acc", 0.0)),
        "non_major_province_exact_acc": float(adopted_full.get("non_major_province_exact_acc", 0.0)),
        "major_province": adopted_full.get("major_province"),
        "major_province_ratio": float(adopted_full.get("major_province_ratio", 0.0)),
    },
}
