...
SAVE_DIR=/home/wzzz/LPRNet/experiments/green_ablation/ABL1_rear_only_030

mkdir -p /home/wzzz/LPRNet/experiments/green_ablation
rm -rf "$SAVE_DIR"
mkdir -p "$SAVE_DIR"

echo "[RUN] ABL-1: rear-only = 0.30, first=0"
$PY train_LPRNet.py \
  --cuda true \
  --data_mode manifest \
