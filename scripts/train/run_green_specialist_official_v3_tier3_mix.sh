...
TRAIN_MANIFEST=/home/wzzz/LPRNet/manifests/unified_manifest_green_balance_round2_greenonly_b_anhui40_noleak_edgefit_tier3_v3_su_conservative_fixed.csv
EVAL_MANIFEST=/home/wzzz/LPRNet/manifests/unified_manifest_green_balance_aggr_v1.csv
SAVE_DIR=/home/wzzz/LPRNet/experiments/green_specialist_official_v3_tier3_mix

mkdir -p /home/wzzz/LPRNet/experiments
rm -rf "$SAVE_DIR"
mkdir -p "$SAVE_DIR"

echo "[RUN] green_specialist_official_v3_tier3_mix"
echo "[INIT] $INIT"
...
$PY src/evaluation/eval_green8_metrics_by_source.py \
  --model "$SAVE_DIR/Final_LPRNet_model.pth" \
  --manifest "$EVAL_MANIFEST" \
  --out_json "$SAVE_DIR/eval_green8_metrics_by_source.json" \
  --batch_size 300 \
  --num_workers 4 \
  2>&1 | tee "$SAVE_DIR/eval_green8_metrics_by_source.log"
