...
SAVE_DIR=/home/wzzz/LPRNet/experiments/green_e21a_cluster2_partial_unfreeze_backbone20_e20adata_e12c_stage2
...
echo "[RUN] E21A E20A-data + partial-unfreeze backbone.20 on E12C stage2"
echo "[INIT] $INIT"
...
$PY src/training/train_LPRNet.py \
  ...
  --trainable_backbone_prefixes backbone.20 \
...
$PY src/utils/update_training_status.py \
  ...
  --process-keyword green_e21a_cluster2_partial_unfreeze_backbone20_e20adata_e12c_stage2 \
  --final-weight "$SAVE_DIR/Final_LPRNet_model.pth" \
  --eval-json "$SAVE_DIR/eval_family_aware.json" \
  --out-json "$STATUS_JSON" \
  --out-md "$STATUS_MD"

echo "[DONE] E21A complete"
