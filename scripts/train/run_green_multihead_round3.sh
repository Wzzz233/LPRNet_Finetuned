...
ROOT_EXP=/home/wzzz/LPRNet/experiments/green_multihead_round3
mkdir -p "$ROOT_EXP"
...
run_one() {
  local name="$1"
  local max_epoch="$2"
  local selection_proxy_samples="$3"
  shift 3
...
run_one A_green_head_only_warmup_e2 2 2000
run_one B_green_head_only_warmup_e5 5 2000
run_one C1_green_partial_unfreeze_upper_c1_e1 1 2000 --trainable_backbone_prefixes backbone.20
run_one C2_green_partial_unfreeze_upper_c2_e1 1 2000 --trainable_backbone_prefixes backbone.16,backbone.20

echo "[DONE] round3 experiments complete"
