#!/bin/bash
# Green CCPD2019 CV-replace v2 smoke training script.
# Uses train_balanced_combined.csv (province-balanced).
# Usage: bash scripts/train/run_green_ccpd2019_cvreplace_v2_smoke.sh

DATE_TAG="20260508"
EXP_DIR="experiments/green_ccpd2019_tilt_db_challenge_cvreplace_v2_${DATE_TAG}_smoke"
mkdir -p "$EXP_DIR"
...
python3 -u src/training/train_LPRNet.py \
  ...
  --preflight_only True \
  --train_batch_size 8 --num_workers 2 \
  --cuda True 2>&1 | tee "${EXP_DIR}/preflight.log"
...
echo ""
echo "=== Step 2: 50-step smoke training ==="
python3 -u src/training/train_LPRNet.py \
  --data_mode manifest \
  --train_manifest "manifests_rebased/green_ccpd2019_tilt_db_challenge_cvreplace_v2_${DATE_TAG}/train_balanced_combined.csv" \
  --test_manifest "manifests_rebased/green_ccpd2019_tilt_db_challenge_cvreplace_v2_${DATE_TAG}/val_cvreplace_v2.csv" \
  ...
  --save_interval 25 --test_interval 25 2>&1 | tee "${EXP_DIR}/train.log"

echo ""
echo "=== Smoke training complete ==="
grep -E "Final test|Training Done" "${EXP_DIR}/train.log" 2>/dev/null || echo "(check train.log)"
