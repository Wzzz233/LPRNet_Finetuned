...
ROOT_EXP=/home/wzzz/LPRNet/experiments/green_multihead_final
mkdir -p "$ROOT_EXP"
...
run_one() {
  ...
  local max_epoch="$2"
  local batch_size="$3"
  local selection_proxy_samples="$4"
  shift 4
  local save_dir="$ROOT_EXP/$name"
  rm -rf "$save_dir"
  mkdir -p "$save_dir"
  echo "[RUN] $name (epochs=$max_epoch, batch=$batch_size)"
  $PY train_LPRNet.py \
...
# F1: D + 10 epoch (batch=64, same as original D)
echo "=========================================="
echo "Starting F1: D + 10 epoch"
echo "=========================================="
run_one F1_D_10epoch 10 64 2000

# F2: D + batch=128 (5 epoch, to compare with D/E settings)
echo "=========================================="
echo "Starting F2: D + batch=128"
echo "=========================================="
run_one F2_D_batch128 5 128 2000

echo "[DONE] F1 and F2 experiments complete"
