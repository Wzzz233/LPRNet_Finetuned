...
SAVE_DIR=/home/wzzz/LPRNet/experiments/green_h32/H32B_classbalanced_plus_focalctc
BASELINE_JSON=/home/wzzz/LPRNet/experiments/green_h32/H32A_classbalanced_firstchar/eval_green8_metrics_only.json

mkdir -p /home/wzzz/LPRNet/experiments/green_h32
rm -rf "$SAVE_DIR"
mkdir -p "$SAVE_DIR"

# Preflight: parameter sanity and baseline availability
case "proxy_exact" in
...
if [ ! -f "$BASELINE_JSON" ]; then
  echo "[FATAL] missing H32A baseline json: $BASELINE_JSON"
  exit 3
fi

echo "[RUN] H32-B: H32A + Focal CTC (main CTC + rear_seq_aux)"
echo "[CONFIG] Keep H32A fixed; only switch ctc_loss_type=standard -> focal"
echo "[SUCCESS CRITERIA] exact > H32A, 苏 first_char >= H32A, macro_exact >= H32A, pos3plus >= H32A"
