    ...
    if os.path.exists(txt_file):
        with open(txt_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if len(parts) == 2:
                    rel_path = normalize_label_rel_path(parts[0])
...
def normalize_label_rel_path(raw_rel_path):
    rel_path = (raw_rel_path or '').strip().replace('\\', '/')
...
def calc_warp_padding_ratio(pts):
    quad = order_quad_points(pts)
    ...
    w_ref = max(max(w_top, w_bottom), 1.0)
    h_ref = max(max(h_left, h_right), 1.0)
    skew_w = abs(w_top - w_bottom) / w_ref
    skew_h = abs(h_left - h_right) / h_ref
    skew = max(skew_w, skew_h)
    pad_w = 0.08 + 0.10 * skew
    pad_h = 0.16 + 0.20 * skew
    pad_w = max(0.08, min(0.28, pad_w))
    pad_h = max(0.16, min(0.42, pad_h))
    return float(pad_w), float(pad_h)


def expand_quad_for_ocr(pts, img_w, img_h, pad_w_ratio, pad_h_ratio):
    q = order_quad_points(pts).astype(np.float32)
    center = np.mean(q, axis=0)

    # C side: ux=(TR-TL)+(BR-BL), vx=(BL-TL)+(BR-TR)
    ...
    half_w = 0.25 * (w_top + w_bottom) * (1.0 + float(pad_w_ratio))
    half_h = 0.25 * (h_left + h_right) * (1.0 + float(pad_h_ratio))
    half_w = max(1.0, half_w)
    half_h = max(1.0, half_h)

    out = np.zeros((4, 2), dtype=np.float32)
    out[0] = center - ux * half_w - vx * half_h  # TL
    out[1] = center + ux * half_w - vx * half_h  # TR
    out[2] = center + ux * half_w + vx * half_h  # BR
    out[3] = center - ux * half_w + vx * half_h  # BL
    return clip_quad_to_image(out, img_w, img_h)


def warp_quad_to_rect(image, pts, dst_w=None, dst_h=None, pad_ratio=0.0, dynamic_pad=False):
    img_h, img_w = image.shape[:2]
    quad = clip_quad_to_image(pts, img_w, img_h)
    if dynamic_pad:
        pad_w, pad_h = calc_warp_padding_ratio(quad)
        if pad_ratio > 0.0:
            pad_w += float(pad_ratio)
            pad_h += float(pad_ratio)
        quad = expand_quad_for_ocr(quad, img_w, img_h, pad_w, pad_h)
    elif pad_ratio > 0.0:
        quad = expand_quad(quad, pad_ratio)
...
def resize_bgr_with_kernel(src, dst_w, dst_h, kernel):
    ...
    if src_w <= 0 or src_h <= 0 or dst_w <= 0 or dst_h <= 0:
        return np.zeros((max(0, dst_h), max(0, dst_w), 3), dtype=np.uint8)
    ys = (np.arange(dst_h, dtype=np.int32) * src_h) // dst_h
    xs = (np.arange(dst_w, dtype=np.int32) * src_w) // dst_w
...
def prepare_board_ocr_input_from_quad_bgr888(
    ...
    warped, ordered_quad, matrix = warp_quad_to_rect(
        image,
        quad,
...
class CCPDBoardDataLoader(Dataset):
    def __init__(
        ...
        self.ocr_quad_pad_ratio = ocr_quad_pad_ratio
    ...
    def __getitem__(self, index):
        ...
        if quad is None and self.ocr_crop_mode == 'obb_warp':
            quad = box_to_quad(bbox)
        if self.ocr_crop_mode == 'obb_warp' and quad is not None:
            crop_bgr, occ_ratio, _, _, _ = prepare_board_ocr_input_from_quad_bgr888(
                image,
                quad,
                self.img_size[0],
                self.img_size[1],
                ...
                quad_pad_ratio=self.ocr_quad_pad_ratio,
            )
        else:
...
class BoardDumpDataLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for c in text_label:
            label.append(CHARS_DICT[c])
        family = (row.get('family') or 'normal7').strip() or 'normal7'
        return image, label, len(label), family
...
class UnifiedManifestDataset(Dataset):
    def __init__(
        ...
        self.plate_box_aug_y = plate_box_aug_y
        self.plate_box_aug_min_iou = plate_box_aug_min_iou
        self.brightness_aug_max = brightness_aug_max
        self.gray3_prob = gray3_prob
        ...
        with open(manifest_path, 'r', encoding='utf-8', newline='') as f:
            ...
            for row in reader:
                ...
                self.img_paths.append(row['img_path'])
                self.img_labels.append(row['text'])

    def __len__(self):
        return len(self.records)

    def _transform_plain(self, image):
        if image.shape[1] != self.img_size[0] or image.shape[0] != self.img_size[1]:
            image = cv2.resize(image, self.img_size)
        # E8 augmentation: brightness shift and gray3 (applied before normalize)
        if self.brightness_aug_max > 0:
            delta = np.random.uniform(0, self.brightness_aug_max)
            image = np.clip(image.astype('float32') + delta, 0, 255).astype('uint8')
        if self.gray3_prob > 0 and np.random.random() < self.gray3_prob:
    ...
    def _resolve_img_path(self, raw_path):
        if os.path.isabs(raw_path):
            return raw_path
        return os.path.join(self.dataset_root, raw_path)

    def __getitem__(self, index):
        row = self.records[index]
        raw_img_path = row['img_path']
        text_label = row['text']
        preprocess_group = (row.get('preprocess_group') or 'plain_plate').strip()

        img_path = self._resolve_img_path(raw_img_path)
        ...
        if preprocess_group == 'board_dump':
            ...
            image = cv2.imread(img_path)
            if image is None:
                msg = f'Failed to read image: {img_path} (from manifest: {raw_img_path})'
                if self.strict_path_check:
                    raise RuntimeError(msg)
                # fallback: try raw path for backward compatibility
                image = cv2.imread(raw_img_path)
                if image is None:
                    raise RuntimeError(f'{msg}; also tried raw path: {raw_img_path}')
            if preprocess_group == 'ccpd_board':
                rel_path = row.get('img_rel_path', img_path)
                img_h, img_w = image.shape[:2]

                # 优先从 manifest row 中读取 quad（CRPD 等外部标注）
                quad = None
                try:
                    q_keys = ['quad_1x', 'quad_1y', 'quad_2x', 'quad_2y',
                ...
                crop_mode = row.get('ocr_crop_mode') or self.ocr_crop_mode
                resize_mode = row.get('ocr_resize_mode') or self.ocr_resize_mode
                resize_kernel = row.get('ocr_resize_kernel') or self.ocr_resize_kernel
                preproc_mode = row.get('ocr_preproc') or self.ocr_preproc
                channel_order = row.get('ocr_channel_order') or self.ocr_channel_order
                min_occ_ratio = float(row.get('ocr_min_occ_ratio') or self.ocr_min_occ_ratio)
                quad_pad_ratio = float(row.get('ocr_quad_pad_ratio') or self.ocr_quad_pad_ratio)

                if crop_mode == 'obb_warp' and quad is None:
                    quad = box_to_quad(bbox)
                if crop_mode == 'obb_warp' and quad is not None:
                    image, _, _, _, _ = prepare_board_ocr_input_from_quad_bgr888(
                        image,
                        quad,
                        self.img_size[0],
                        self.img_size[1],
                    ...
                    occ_ratio = estimate_ocr_occ_ratio(crop_box.w, crop_box.h, self.img_size[0], self.img_size[1], resize_mode)
                    if min_occ_ratio > 0.0 and occ_ratio < min_occ_ratio and crop_mode != 'tight':
                        if crop_mode == 'match':
                            recrop = compute_match_ytrim_crop(crop_box, self.img_size[0], self.img_size[1], min_occ_ratio)
                            if recrop is not None:
                                crop_box = clamp_box(recrop, img_w, img_h)
                        else:
                            crop_box = compute_expand_crop_box(bbox, img_w, img_h, 0.08, 0.16)
                    crop_bgr = image[crop_box.y1:crop_box.y2 + 1, crop_box.x1:crop_box.x2 + 1]
                    image, _ = prepare_board_ocr_input_bgr888(
                        crop_bgr,
                        self.img_size[0],
                        self.img_size[1],
                        resize_mode,
                ...
                # E8 augmentation: brightness shift and gray3 (applied before normalize)
                if self.brightness_aug_max > 0:
                    delta = np.random.uniform(-self.brightness_aug_max, self.brightness_aug_max)
                    image = np.clip(image + delta, 0, 255)
                if self.gray3_prob > 0 and np.random.random() < self.gray3_prob:
                    gray = cv2.cvtColor(image.astype('uint8'), cv2.COLOR_BGR2GRAY)
                    image = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR).astype('float32')
                image -= 127.5
                image *= 0.0078125
                image = np.transpose(image, (2, 0, 1))
            else:
                image = self._transform_plain(image)
