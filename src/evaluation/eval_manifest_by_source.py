...
def main():
    ...
    stats = defaultdict(lambda: {'count': 0, 'exact': 0, 'first': 0, 'char_ok': 0, 'char_total': 0, 'family': '', 'bad_cases': []})
    seen = 0
    with torch.no_grad():
        for images, labels, lengths, families in loader:
            ...
            for row, fam, gt, pred in zip(batch_records, sample_families, targets, preds):
                ...
                if len(st['bad_cases']) < 10 and pred != gt:
                    st['bad_cases'].append({
    ...
    out = {'model': args.model, 'manifest': args.manifest, 'decode_mode': 'family_aware_beam', 'by_source': {}}
    for bucket, st in sorted(stats.items()):
        out['by_source'][bucket] = {
            'family': st['family'],
