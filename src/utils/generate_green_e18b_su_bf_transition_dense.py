...
TARGET_BUCKET_PLAN = [
    ...
    ('board_mid_occ', 210),
    ('board_low_occ', 180),
    ('board_extreme_tail', 90),
]


def parse_bucket_plan(text):
    if not text:
        return list(TARGET_BUCKET_PLAN)
    out = []
    for part in text.split(','):
        ...
        if key not in {'geometry_clean', 'board_mid_occ', 'board_low_occ', 'board_extreme_tail'}:
            raise ValueError(f'unknown bucket in --bucket-plan: {key}')
        if value < 0:
            raise ValueError(f'negative bucket count in --bucket-plan: {part}')
        out.append((key, value))
    if not out:
        raise ValueError('empty --bucket-plan after parsing')
    return out
...
def main():
    ap = argparse.ArgumentParser(description='Generate targeted dense append set on top of a base manifest using dump text anchors.')
    ap.add_argument('--base-manifest', required=True)
    ap.add_argument('--out-dir', required=True)
    ap.add_argument('--out-manifest', required=True)
    ap.add_argument('--dataset-name', default='green_e18b_su_bf_transition_dense_600')
    ap.add_argument('--source-name', default='e18b_su_bf_transition_dense_600')
    ...
    ap.add_argument('--target-texts', default='', help='comma-separated gt_text allowlist; empty means all valid 8-char texts from dump csvs')
    ap.add_argument('--bucket-plan', default='', help='comma-separated bucket plan, e.g. board_low_occ=120,board_extreme_tail=120')
    args = ap.parse_args()
    ...
    target_text_allow = {x.strip() for x in (args.target_texts or '').split(',') if x.strip()}
    if target_text_allow:
        targets = [x for x in targets if x['gt_text'] in target_text_allow]
    if not targets:
        raise RuntimeError('no valid 8-char gt_text found in dump csvs')
    target_bucket_plan = parse_bucket_plan(args.bucket_plan)
    ...
    summary = {
        ...
        'target_bucket_plan': {k: v for k, v in target_bucket_plan},
