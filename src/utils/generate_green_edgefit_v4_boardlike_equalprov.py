...
def train_target_by_province_equal(total_per_province):
    return {p: int(total_per_province) for p in ALL_PROVINCES}
...
def generate_dataset(args):
    ...
    if args.train_per_province_total is not None:
        total_per_province = int(args.train_per_province_total)
        train_targets = train_target_by_province_equal(total_per_province)
        bucket_items = list(quotas['train'].items())
        bucket_sum = sum(v for _, v in bucket_items)
        for province in ALL_PROVINCES:
            remaining = total_per_province
            for idx, (bucket, total) in enumerate(bucket_items):
                if idx == len(bucket_items) - 1:
                    count = remaining
                else:
                    count = int(round(total_per_province * total / max(1, bucket_sum)))
                    count = min(count, remaining)
                train_bucket_plan[province][bucket] = count
                remaining -= count
            if remaining != 0:
                train_bucket_plan[province][bucket_items[-1][0]] += remaining
    else:
...
def parse_args():
    ...
    ap.add_argument('--train_per_province_total', type=int, default=None)
    for split in ['train', 'val', 'test']:
        for bucket in ['geometry_clean', 'board_mid_occ', 'board_low_occ', 'board_extreme_tail']:
