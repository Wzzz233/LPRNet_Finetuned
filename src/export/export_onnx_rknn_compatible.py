...
from LPRNet import small_basic_block
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--keys-file", default="", help="Path to keys file (one char per line); overrides CHARS.")
...
def main() -> int:
    ...
    # Override CHARS from keys file if provided
    if args.keys_file:
        custom_chars = [l.strip() for l in open(args.keys_file, 'r', encoding='utf-8') if l.strip()]
        ...
        # Note: top-level CHARS (from load_data) points to same in-place modified list
        print(f'[KeysFile] Overrode CHARS: {len(custom_chars)-1} real keys -> class_num={len(custom_chars)}')
