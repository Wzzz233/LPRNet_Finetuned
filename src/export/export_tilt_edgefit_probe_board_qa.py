...
PROBE = Path('/home/wzzz/LPRNet/suhu_tilt_edgefit_probe_20260403')
META = PROBE / 'details' / 'accepted.json'
OUT = Path('/home/wzzz/LPRNet/qa_tilt_edgefit_probe_board_processed_20260403')
...
def main():
    ...
    for row in rows:
        ...
        header = make_text([
            ...
            'top-left: original exact-quad synthetic | top-right: stronger-tilt with edge-fitted true quad',
            f'bottom-left: board input original (occ={occ_src:.4f}) | bottom-right: board input edge-fitted aug (occ={occ_aug:.4f})',
        ], width)
    ...
    (OUT / 'README.txt').write_text(
        '四宫格：左上原始exact-quad synthetic，右上更强倾斜且quad重新严格贴边后的增强图；左下原始板端94x24输入，右下增强后板端94x24输入。重点审查：新quad是否真正贴边、是否还漏字、是否比上一版更少露出边缘。\n',
        encoding='utf-8'
    )
    print(str(OUT))
