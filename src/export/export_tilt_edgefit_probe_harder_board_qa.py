...
PROBE = Path('/home/wzzz/LPRNet/suhu_tilt_edgefit_probe_harder_20260403')
META = PROBE / 'details' / 'accepted.json'
OUT = Path('/home/wzzz/LPRNet/qa_tilt_edgefit_probe_harder_board_processed_20260403')
...
def main():
    ...
    for row in rows:
        ...
        vis_src = src.copy(); vis_aug = aug.copy()
        cv2.polylines(vis_src, [np.asarray(quad_src, dtype=np.int32).reshape(-1,1,2)], True, (0,255,0), 1)
        cv2.polylines(vis_aug, [np.asarray(quad_aug, dtype=np.int32).reshape(-1,1,2)], True, (0,255,0), 1)
        ...
        a = np.full((h1, top_l.shape[1], 3), 255, dtype=np.uint8); a[:top_l.shape[0], :top_l.shape[1]] = top_l
        b = np.full((h1, top_r.shape[1], 3), 255, dtype=np.uint8); b[:top_r.shape[0], :top_r.shape[1]] = top_r
        ...
        c = np.full((h2, bot_l.shape[1], 3), 255, dtype=np.uint8); c[:bot_l.shape[0], :bot_l.shape[1]] = bot_l
        d = np.full((h2, bot_r.shape[1], 3), 255, dtype=np.uint8); d[:bot_r.shape[0], :bot_r.shape[1]] = bot_r
        ...
        header = make_text([
            ...
            'top-left: original exact-quad synthetic | top-right: harder edge-fitted tilt+realism demo',
            f'bottom-left: board input original (occ={occ_src:.4f}) | bottom-right: board input harder demo (occ={occ_aug:.4f})',
        ], width)
    ...
    (OUT / 'README.txt').write_text(
        '四宫格：左上原始exact-quad synthetic，右上更高难度edgefit增强；左下原始板端94x24输入，右下更高难度增强后的板端94x24输入。重点看：相较简单难度是否更像真实、是否仍保持quad贴边、是否开始伤字符。\n',
        encoding='utf-8'
    )
    print(str(OUT))
