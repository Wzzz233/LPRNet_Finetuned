...
EDGEFIT_ROOT = Path('/home/wzzz/LPRNet/green_edgefit_tier3_full_v3_su_conservative')
OUT = Path('/home/wzzz/LPRNet/manifests/unified_manifest_green_balance_round2_greenonly_b_anhui40_noleak_edgefit_tier3_v3_su_conservative.csv')
...
if synthetic_template is None:
    raise RuntimeError('No synthetic template found')
...
for split in ['train', 'val', 'test']:
    ...
    for line in txt.read_text(encoding='utf-8').splitlines():
        ...
        row['img_rel_path'] = f'green_edgefit_tier3_full_v3_su_conservative/{rel}'
        row['dataset_name'] = 'green_edgefit_tier3_full_v3_su_conservative'
        row['split'] = split
        row['text'] = text.strip().upper()
        row['plate_len'] = str(len(row['text']))
        row['family'] = 'green8'
        row['sub_type'] = 'green_small'
        row['source'] = 'synthetic_exact_quad_edgefit_tier3_v3_su_conservative'
        row['is_real'] = '0'
        row['need_tilt_aug'] = '1'
        ...
        row['bbox_source'] = 'synthetic_exact_quad_edgefit_tier3_v3_su_conservative'
        row['quad_source'] = 'synthetic_exact_quad_edgefit_tier3_v3_su_conservative'
...
report = {
    ...
    'train_su_count_after_merge': sum(1 for r in all_train if (r.get('text') or '').startswith('苏')),
