...
def main():
    ap = argparse.ArgumentParser(description='Append dumplike board-dump rows onto an E2 manifest without removing any existing rows.')
    ...
    bad_groups = [r.get('img_rel_path') for r in append_train if (r.get('preprocess_group') or '') != 'board_dump']
    if bad_groups:
        raise RuntimeError(f'append manifest contains non-board_dump rows, example={bad_groups[:3]}')
    ...
    append_groups = Counter((r.get('preprocess_group') or '') for r in append_train)
    ...
    summary = {
        ...
        'append_preprocess_group_counts': dict(append_groups),
        'append_examples': [
            {
                'img_rel_path': r.get('img_rel_path'),
                'text': r.get('text'),
                'source': r.get('source'),
                'preprocess_group': r.get('preprocess_group'),
            }
